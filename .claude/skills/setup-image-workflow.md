...
### ⚠️ ヒアリング運用ルール (2026-06-11 追加)

**13 項目は省略禁止**。 設計書 / 既存ヒアリングから自動導出できる項目も、 必ず ZAS OPS が「推定: ○○」 と user に提示して承認を取る。 user の手間は「OK」 連打でも構わないが、 **項目自体をスキップしないこと**。

なぜ:
- 自動導出が間違っている可能性 (案件特有の要望が反映されない)
- user の判断機会を奪うと品質が下がる
- ヒアリングプロセスの検証もできなくなる
...
```
1. 使用用途
   → ZAS OPS 推定: About インテリア
   → user 確認 [OK / 修正]
2. 参考画像 URL
   → ZAS OPS 推定: lamune-kyoto.com + 既存 hero.jpg
   → user 確認 [OK / 修正]
...
```
...
ZAS OPS は user に以下を順番に聞いてプロンプトを作成する。 設計書 / IMAGE-REQUESTS から推定できる項目も省略せず、 推定値を提示して user 承認を取る:
